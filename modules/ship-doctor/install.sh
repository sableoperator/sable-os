...
SOURCE_SCRIPT="$MODULE_DIR/bin/ship-doctor"

INSTALL_DIR="$HOME/.local/bin"
TARGET_SCRIPT="$INSTALL_DIR/ship-doctor"

BACKUP_ROOT="$HOME/SableOS_Backups"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP_DIR="$BACKUP_ROOT/ship-doctor-$TIMESTAMP"
...
Installs:
  ship-doctor -> ~/.local/bin/ship-doctor
USAGE
}

log() {
  echo "[ship-doctor installer] $1"
}
...
log "Preparing Ship Doctor install."
echo
...
planned "Copy Ship Doctor script to: $TARGET_SCRIPT"
...
if [[ -f "$TARGET_SCRIPT" ]]; then
  mkdir -p "$BACKUP_DIR"
  cp "$TARGET_SCRIPT" "$BACKUP_DIR/ship-doctor"
  log "Existing command backed up to: $BACKUP_DIR/ship-doctor"
fi

cp "$SOURCE_SCRIPT" "$TARGET_SCRIPT"
chmod +x "$TARGET_SCRIPT"

log "Installed Ship Doctor to: $TARGET_SCRIPT"
